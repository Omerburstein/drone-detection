...
- [ ] 2026-08-24 — [deploy] **Get real frames from the drone's own downlink.** The live path (`src.live_detect`) is written and tested but **has never seen the intended source**. As of 2026-08-24 the FPV goggles do not enumerate on this laptop in any device class — not camera, not mass storage — over USB-C-to-USB; the goggles' HDMI is input-only, so there is no video-out path yet. Until that is resolved every retention figure the live path prints is an ARD-MAV/ARD100 number standing in for a camera nobody has measured, and [edge-budget.md §4.3](edge-budget.md) projects a wide-FOV low-resolution feed to near-zero recall on optics alone. **Trigger:** the goggles expose a UVC device or a removable drive, or an HDMI capture card is bought. Cheap version — copy one recording off the goggles' SD card and run `src.live_detect --source <file>`, which needs no capture hardware at all.
