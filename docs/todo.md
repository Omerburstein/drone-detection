...
- [ ] 2026-09-17 — [algo] **Test the polarity hypothesis — run GLAD on an inverted copy of the FIELD capture.** Measured 2026-09-17 ([glad-model.md](glad-model.md) §5b): ARD-MAV's ground truth is **84.4% brighter than its background** — white DJI Phantoms shot against roads, concrete and grass — while our target is **98.5% darker** (a dark airframe silhouetted against sky) and our ground false alarms are **99.1% brighter**, i.e. sitting in the training distribution our own drone falls outside of. That is a correlation plus an inference; **inverting the image tests it directly.** Feed `255 - pixel` and the polarities swap: our drone joins the training side, the pale clutter leaves it. If the prior is what drives this, cold acquisition (`global yolo`, currently 0.2% of frames) should rise sharply and the clutter locks should not form. ~20 min on this host, **no labels needed** — the branch mix alone answers it, and it is the cheapest discriminating experiment currently available. Needs a `--invert` flag or a pre-inverted copy under `data/processed/`; prefer the flag, since a second 388 MB video to hold one arithmetic operation is not worth the disk.
